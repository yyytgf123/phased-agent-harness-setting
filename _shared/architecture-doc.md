...
## 무엇을 담나 (섹션 고정)
골격은 `../templates/architecture.md.tmpl` (6섹션: Agents/Skills/Orchestrator/Hooks/Data Flow + 최종갱신 헤더).
이 문서는 그 골격을 **어떻게 유지·갱신하나**를 정한다.
...
> **층위 주의:** "구조 바뀌면 갱신"이 동작하려면 Phase 3에서 이 갱신 규칙이 루트 `CLAUDE.md`의
> `## Docs & Work Orders` 섹션에 박혀야 한다(원칙: `design-principles.md` §7). 파일 자체는 Phase 5에서 최초 생성.

# Safety Rules — 프로젝트 단일 소스

> CLAUDE.md `## NEVER`는 이 파일의 4줄 요약 + 포인터. 에이전트·스킬은 표준 응답을 여기서 참조한다.
...
- DB 마이그레이션 실행 (flyway migrate, liquibase update) → 작성까지만
...
## 범위 침범 차단
- 앱 에이전트: infra/, monitoring/ 수정 금지 / 인프라 에이전트: app/ 소스 수정 금지
